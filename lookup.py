...
class InFeatureClass:
    ...
    def __repr__(self):
        return '<OpentypeObject inFeatureClass \'{0}\' with {1} members>'.format(self.name, len(self.members))
...
class LookupFlag:
    def __init__(self, code=None, script=None, language=None):
        self.name = code.partition(' ')[0]
        self.code = code
        self.script = script
        self.language = language
        self.lookup_reference = None
        self.is_comment = False

        if code is not None:
            self.parse_code()

        self.is_lookupflag = not any([self.script, self.language, self.lookup_reference, self.is_comment])

    def __repr__(self):
        return '<OpentypeObject LookupFlag{type} \'{code}...\'>'.format(
            type=' -Comment-' if self.is_comment else '',
            code=self.code.strip()[:22],
        )
    ...
    def subset(self, scripts=None, languages=None, glyphs=None):
        ...
        if scripts and self.script in scripts:
            return new

        if languages and self.language in languages:
            return new

        # If this is an actual lookupFlag and has no associated script or language, keep it.
        if self.is_lookupflag:
            return new
...
class Lookup:
    def __init__(self, code=None, name=None, script=None, language=None):
        self.raw_code = code
    ...
    def __repr__(self):
        return '<OpentypeObject Lookup \'{0}\' with {1} lines covering scripts {2}>'.format(self.name, len(self.code_sequence), sorted(self.scripts))
    ...
    def parse_code(self):
        ...
        code_lines = [x.strip() for x in self.lookup_code.split('\n') if x.strip()]
        ...
        for line in code_lines:
            ...
            if not line:
                ...
                obj = LookupFlag(line, script=self.current_script, language=self.current_language)

            elif line.startswith('@'):
                obj = InFeatureClass(line, script=self.current_script, language=self.current_language)
                self.all_glyphs.update(set(obj.members))

            elif line.startswith('sub'):
                obj = Substitution(line, script=self.current_script, language=self.current_language)
                ...
                if obj.script is not None:
                    ...
                    self.current_script = obj.script
                    self.current_language = None

                if obj.language is not None and self.current_script is not None:
                    self.languages.add(obj.language)
                    self.current_language = obj.language

            if self.current_script is not None:
                obj.script = self.current_script
            if self.current_language is not None:
                obj.language = self.current_language

            self.code_sequence.append(obj)

    def write(self, tab_level=0):
        code = '\n'.join(x.write(tab_level=tab_level + 1) if hasattr(x, 'write') else x for x in self.code_sequence)
        lookup_text = '{tab}lookup {lookup_name} {{\n{code}\n{tab}}} {lookup_name};\n'.format(
    ...
    def subset(self, scripts=None, languages=None, glyphs=None):
        ...
        # Check to make sure a substitution is in the lookup, otherwise the feature is redundant
...
\t@Markscomb = [acute breve caron circumflex dieresis dotaccent grave macron ring tilde breveinvertedcomb horncomb hungarumlaut];
\t@MarkscombCase = [acute.case breve.case caron.case circumflex.case dieresis.case dotaccent.case grave.case macron.case ring.case tilde.case breveinvertedcomb.case horncomb.case hungarumlaut.case];
...
\tlookupflag UseMarkFilteringSet @rephDeva;
\tsub @DevaIMatras' @DevaFullforms [reph-deva] by @DevaIMatraRephs;
\tsub @DevaIMatras' @DevaFullforms [anusvara-deva] by @DevaIMatraAnusvaras;
\tsub @DevaIMatras' @DevaFullforms [reph_anusvara-deva] by @DevaIMatraRephAnusvaras;
\tsub [@DevaIMatraRephs @DevaIMatraRephAnusvaras @DevaIMatraAnusvaras] @DevaFullforms' lookup removeReph [anusvara-deva reph-deva reph_anusvara-deva]';
} Abvs3_devanagari;
""", script='deva')
    f = y
    print(f.write())
    ...
    # print(f[0].subset(glyphs='circumflexcomb commaturnedabovecomb dblgravecomb'))
